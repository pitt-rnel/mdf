function [res] = subsref(obj,S)
    ...
    if ~isvector(obj) && max(builtin('size',obj)) > 1
        throw(MException('mdfObj:subsref',['Operation not possible. Please transform object in a vector']));
    elseif strcmp(S(1).type,'()')
        % check if the first operation is an array indexing
        % we assume that the user knows the length of the vector
        % if that's the case, we extract the element and appaly the rest of the
        % subsref steps
        if length(S(1).subs) ~= 1
            % check if we got multiple dimensions
            throw(MException('mdfObj:subsref',['Please specify only one index. This object can only be a vector']));
