function varargout = subsasgn(obj,S,V)
        ...
        elseif length(intersect(S(1).subs,fields(obj.metadata)))
            ...
            S = [ ...
                struct{ ...
                    ...
                    'subs', 'metadata' },
                S];
        elseif length(intersect(S(1).subs,obj.def.rf_children.rf_fields))
            % obj.<child-prop>
            % becomes
    ...
    if strcmp(S(1).type,'.')
        if strcmp(S(1).subs,'data') && ( ...
                length(S) == 1 || ...
                ~strcmp(S(2).type,'.') )
            throw(MException('rfObj:subsasgn','Invalid data property assignment'));
        end %if
    end %if
    ...
    if strcmp(S(1).type,'.')
        if strcmp(S(1).subs,'data') && strcmp(S(2).type,'.')
            ...
            j = find(strcmp(obj.def.rf_data.rf_fields,field));
            if isempty(j)
                % new field, append at the end
                obj.def.rf_data.rf_fields{end+1} = field;
                j = length(obj.def.rf_data.rf_fields);
            end %if
